            ...
            try { // non-ivertible matrix (can happen if a triangle is degenerate)
                for (int px=bbminx; px<=bbmaxx; px++) { // rasterize the bounding box
                    for (int py=bbminy; py<=bbmaxy; py++) {
                        ...
                        int intensitySmooth = (int)(coord[0]*intensity0 + coord[1]*intensity1 +coord[2]*intensity2);  //interpolates the colors at the 3 vertices 
                        int color = new Color(intensitySmooth, intensitySmooth, intensitySmooth).getRGB();                    
                        image.setRGB(px, py, color);
                ...
                for (int px=bbminx; px<=bbmaxx; px++) { // rasterize the bounding box
                    for (int py=bbminy; py<=bbmaxy; py++) {
                      ...
                      //UV coords interpolation  
        	            ...
        	            double[] normal0 = {xN[0], yN[0],zN[0]};
        	            int intensity0RED   = (int)Math.min(255, Math.max(0, col.getRed()  *dot_product(normal0, new double[]{0., 0., 1.})));
        	            int intensity0GREEN = (int)Math.min(255, Math.max(0, col.getGreen()*dot_product(normal0, new double[]{0., 0., 1.})));
        	            int intensity0BLUE  = (int)Math.min(255, Math.max(0, col.getBlue() *dot_product(normal0, new double[]{0., 0., 1.})));
        	            
        	            double[] normal1 = {xN[1], yN[1],zN[1]};
        	            int intensity1RED   = (int)Math.min(255, Math.max(0, col.getRed()  *dot_product(normal1, new double[]{0., 0., 1.})));
        	            int intensity1GREEN = (int)Math.min(255, Math.max(0, col.getGreen()*dot_product(normal1, new double[]{0., 0., 1.})));
        	            int intensity1BLUE  = (int)Math.min(255, Math.max(0, col.getBlue() *dot_product(normal1, new double[]{0., 0., 1.})));
        	            
        	            double[] normal2 = {xN[2], yN[2],zN[2]};
        	            int intensity2RED   = (int)Math.min(255, Math.max(0, col.getRed()  *dot_product(normal2, new double[]{0., 0., 1.})));
        	            int intensity2GREEN = (int)Math.min(255, Math.max(0, col.getGreen()*dot_product(normal2, new double[]{0., 0., 1.})));
        	            int intensity2BLUE  = (int)Math.min(255, Math.max(0, col.getBlue() *dot_product(normal2, new double[]{0., 0., 1.})));
            ...
            try { // non-ivertible matrix (can happen if a triangle is degenerate)
                for (int px=bbminx; px<=bbmaxx; px++) { // rasterize the bounding box
                    for (int py=bbminy; py<=bbmaxy; py++) {
                        ...
                        int intensity = (int)Math.min(255, Math.max(0, 255*dot_product(NPix, new double[]{0., 0., 1.}))); 
                ...
                for (int px=bbminx; px<=bbmaxx; px++) { // rasterize the bounding box
                    for (int py=bbminy; py<=bbmaxy; py++) {
                      ...
                      //UV coords interpolation  
                        ...
                        int intensityRED =   (int)Math.min(255, Math.max(0, col.getRed()  *dot_product(NPix, new double[]{0., 0., 1.}))); 
                        int intensityGREEN = (int)Math.min(255, Math.max(0, col.getGreen()*dot_product(NPix, new double[]{0., 0., 1.}))); 
                        int intensityBLUE =  (int)Math.min(255, Math.max(0, col.getBlue() *dot_product(NPix, new double[]{0., 0., 1.}))); 
                ...
                for (int px=bbminx; px<=bbmaxx; px++) { // rasterize the bounding box
                    for (int py=bbminy; py<=bbmaxy; py++) {
                      ...
                      //UV coords interpolation  
        	            ...
        	            NPix[0] = colNormalMap.getRed();
        	            NPix[1] = colNormalMap.getGreen();
        	            NPix[2] = colNormalMap.getBlue();
...
public void SpecularShading(){
        ...
        for (int t=0; t< triangles.length/3; t++) { // iterate through all triangles
            ...
            //float spec = (float) Math.pow(Math.max(dot_product(reflected, new double[]{0.3,0.3,0.}), 0.)  , 5) ;
            //float spec = (float) Math.pow(Math.max(dot_product(reflected, new double[]{0.,0.,1.}), 0.)  , 5) ;
